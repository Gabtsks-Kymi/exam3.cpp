...
// This just helps setting up exercises in the grader.
...
private:
  ...
  struct node {
    ...
    node *left, *right, *parent, *next, *prev_start;

    node(const T &x, node *p = nullptr)
        : data(x), plythos(1), balance(EH), left(nullptr), right(nullptr), parent(p), next(nullptr), 
        prev_start(nullptr) {}
  };
...
private:
 ...
 static node *insert(node *t, const T &x) {
    while (true) {
      if (x < t->data) {
        if (t->left == nullptr){
         // node *lefty = new node(x, t);
         // lefty->prev_start = lefty;
        ...
        if (t->right == nullptr){
          //node *righty = new node(x, t);
         // righty->prev_start = righty;
          return (t->right = new node(x, t));
        }
        else
          t = t->right;
      } else{
         node *p = new node(t->data, nullptr);
         ++(t->plythos);
         p->prev_start = t;
...
private:
  ...
  public:
    ...
    void advance() override {
      ...
      if(ptr->prev_start != nullptr) {
        pass_by = 1;

      if ((ptr->prev_start)->right != nullptr){
        ptr = leftdown((ptr->prev_start)->right);
        pass_by = 1;}
      else{
        ptr = leftup(ptr->prev_start);
        pass_by = 1;}
